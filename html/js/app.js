...
var topPlayers = [];
var translate = {};
var resourceName = "nvrs_playtimeshop";

function nuiPost(endpoint, body) {
    return $.post("https://" + resourceName + "/" + endpoint, body ? JSON.stringify(body) : JSON.stringify({}));
}
...
function setItemsIntoCategory(cate, itemList) {
    cate.forEach((c) => {
        c.items = [];
    });
    cate.forEach((cateElements) => {
        itemList.forEach((itemElements) => {
...
$(document).on("click", ".buyButton", function () {
    var itemData = $(this).attr("data-itemData");
    var jsonData = JSON.parse(itemData);
    var line =
        (translate.purchaseLineStart || "") +
        '<span class="wantCoin">' +
        jsonData.price +
        " " +
        translate.coin +
        "</span>" +
        (translate.purchaseLineEnd || "");
    $(".wantText").html(line);
...
$(document).on("click", "#wantBuyButton", function () {
    var itemData = $(this).attr("data-buyButtonItem");
    var jsonData = JSON.parse(itemData);
    $(".succesItemLabel").html(jsonData.label);
    $("#succesImg").attr("src", jsonData.image);
    if (myCoin >= jsonData.price) {
        nuiPost("buyItem", { itemInfo: jsonData }).done(function (data) {
...
$(document).on("click", ".redeemOkButton", function () {
    var codeInputValue = $("#redeemCodeInput").val().trim();
    var placeholder = translate.redeemPlaceholder || "";
    if (codeInputValue && codeInputValue !== placeholder && codeInputValue.length > 0) {
        nuiPost("sendInput", { input: codeInputValue }).done(function (data) {
            if (data) {
                $(".succesfullyArea").fadeIn(200);
                setTimeout(() => {
                    $(".succesfullyArea").fadeOut(200);
                }, 3000);
                myCoin += parseInt(data, 10);
                $(".creditCount").html(myCoin);
            } else {
...
window.addEventListener("message", (event) => {
    if (event.data.type === "openui") {
        ...
        if (event.data.steamAvatarUrl) {
            $("#profilePhotoX").attr("src", event.data.steamAvatarUrl);
        }
        ...
        $("#exitText").html(translate.exit);
        if (translate.redeemPlaceholder) {
            $("#redeemCodeInput").attr("placeholder", translate.redeemPlaceholder);
        }
        $("#wantCancelButton").html(translate.cancel);
        $("#wantBuyButton").html(translate.buy);
        $(".successfullyText").html(translate.succesfully);
        $(".purchasedText").html(translate.purchased);
        $(".redeemOkButton").html(translate.accept);
    }
});
